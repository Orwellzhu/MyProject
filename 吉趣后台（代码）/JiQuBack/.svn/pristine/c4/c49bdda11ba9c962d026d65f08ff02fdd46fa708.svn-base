	...
	obj.prototype={
			...
			$.ajax({
				...
				"data":{
			  		"filter": {
				    	...
				    	"fields": ["Name","Nickname","Password","HeadImgUrl","AccountNumber","Phone","id","Role","createdAt","HeadImgId"],
		...
		strLink:function(ret,id){
			...
			strBox=['<tr>'
		                ...
		                ,'<th>'
		                	...
		                	,'<button type="button" class="btn btn-danger btn-sm delbtn" data-id="'+ ret.id +'" img-id="'+ret.HeadImgId+'">删除</button> '
		                ,'</th>'
		            ,'</tr>'];
			return strBox.join('');
		},
		...
		bindDelEvent:function(){
			...
			this.$TableContentWrapper.off('click.zcc').on('click.zcc', 'tr > th > .delbtn', function(ev){
				...
				if (isDel){
					_this.delOneData($this.attr('data-id'),$this.attr('img-id'));
				}
			});
		},
		// 加载中
