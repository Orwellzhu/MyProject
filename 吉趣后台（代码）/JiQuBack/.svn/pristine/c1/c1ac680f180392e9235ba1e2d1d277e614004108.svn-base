	...
	obj.prototype={
		...
		strLine:function(ret,i){
			...
			if(state==0){
				...
				button='<a href="#activityManage_Check?id='+ret.id+'"><button type="button" class="btn btn-default btn-sm">审查</button> </a>'+
				'<button type="button" class="btn btn-danger btn-sm"  data-id="'+ ret.id +'" img-id="'+ret.PicId+'">删除</button> ';
				attendButton='<a href="#activityManage_Entered?id='+ret.id+'">  <button type="button" class="btn btn-default btn-xs">查看报名人('+ret.Attend.length+')</button> </a>';
			}else if(state==1){
			...
			var Organizer="";if(ret.Organizer){Organizer=ret.Organizer;}
			var strArray=[
				'<tr>'
	                ,'<th>'+i+'</th>'
	                ,'<th>'+ret.Name+'</th>'
